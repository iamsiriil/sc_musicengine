...
TestMEValidators : UnitTest {
	...
	test_intervalIsValid_correctInput {
		var fixtures = [
			"m2", "m9", "A1", "A8",
			"M2", "M9", "d3", "d10",
			"m3", "m10", "A2", "A9",
		...
		fixtures.do { |f|
			...
			this.assertEquals(
				nil,
				MEValidators.intervalIsValid(f),
				"Testing valid input %. Should return 'nil'.".format(f)
			);
		};
	}
	...
	test_intervalIsValid_wrongtInput {
		var fixtures = [
			"P1", "P8",
			"d2", "d9",
			"P2", "P9",
		...
		fixtures.do { |f|
			...
			this.assertException(
				{ MEValidators.intervalIsValid(f) },
				Error,
				"Testing invalid input %. Sould throw Error.".format(f)
			);
		};
	}
}