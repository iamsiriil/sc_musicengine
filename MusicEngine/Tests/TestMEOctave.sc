...
TestMEOctave : UnitTest {
	...
	test_getClosestOctave_correctInputMiddleOctaves {
		...
		fixtures.do { |f|
			...
			this.assertEquals(
				MEOctave.getClosestOctave(f.midi, f.letter, validate: true),
				f.expected,
	...
	test_getClosestOctave_rangeInvariant {
		...
		(0..127).do { |midi|
			...
			letters.do { |letter|
				result = MEOctave.getClosestOctave(midi, letter.asString, validate: true);
	...
	test_getClosestOctave_deterministic {
		...
		100.do {
			...
			result1 = MEOctave.getClosestOctave(midi, letter, validate: true);
			result2 = MEOctave.getClosestOctave(midi, letter, validate: true);
	...
	test_getClosestOctave_randomCorrectness09 {
		...
		100.do {
			...
			this.assertEquals(
				MEOctave.getClosestOctave(testNote, letter.asString, validate: true),
				expected,
	...
	test_getClosestOctave_randomMIDIValidation {
		...
		100.do { |m|
			...
			this.assertException(
				{ MEOctave.getClosestOctave(pick, "C", validate: true) },
				Error,
	...
	test_checkOctaveCross_crossUnder {
		...
		fixtures.do { |f|
			...
			this.assertEquals(
				MEOctave.checkOctaveCross(f.noteName, validate: true),
				f.expected,
	...
	test_checkOctaveCross_invalidNoteName {
		...
		fixtures.do { |f|
			...
			this.assertException(
				{ MEOctave.checkOctaveCross(f, validate: true) },
				Error,
	...
	test_getOctave_validInputFirstOctave {
		...
		fixtures.do { |f|
			...
			this.assertEquals(
				MEOctave.getOctave(f, validate: true),
				-1,
	...
	test_getOctave_validInputCFlat {
		...
		fixtures.do { |f|
			...
			this.assertEquals(
				MEOctave.getOctave(f.midiNote, f.noteName, validate: true),
				f.expected,
